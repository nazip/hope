...
$(document).ready(function () {
  ...
  .on("ajax:success", function(evt, data, status, xhr){
    $("." + data.obj.electable_type + "-" + data.obj.electable_id).html("Рейтинг: " + data.likes);
      ...
      {
        $(first).text("dislike");
        $(first).attr("href", ((data.obj.electable_type.toLowerCase() == "question") ? get_href(data.obj.electable_id, -1) : get_href(-1, data.obj.electable_id))
           + "?like=-1");
        $(first).attr("data-method", "post");
        $(second).text("cancel");
        $(second).attr("href", "/elects/"+data.obj.id
        + ((data.obj.electable_type.toLowerCase() == "question") ? "?question_id=" : "?answer_id=")+ data.obj.electable_id);
      ...
      else if(data.action == 0)
      {
        ...
        $(second).text("dislike");
        $(second).attr("href", ((data.obj.electable_type.toLowerCase() == "question") ? get_href(data.obj.electable_id, -1) : get_href(-1, data.obj.electable_id))
           + "?like=-1");
